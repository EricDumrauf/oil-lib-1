...
import lxml.etree
#import genshi
from ....package import genshi
#import genshi.output
from ....package.genshi import output
#from genshi.template import MarkupTemplate
from ....package.genshi.template import MarkupTemplate
#from genshi.filters import Transformer
from ....package.genshi.filters import Transformer
#from genshi.filters.transform import ENTER, EXIT
from ....package.genshi.filters.transform import ENTER, EXIT
#from genshi.core import Stream
from ....package.genshi.core import Stream
#from genshi.template.interpolation import PREFIX
from ....package.genshi.template.interpolation import PREFIX


from .base import AerooStream
from ..reporting import Report, MIMETemplateLoader
...
class Template(MarkupTemplate):
    ...
    def _aeroo_statements(self, tree):
        ...
        for statement in tree.xpath(s_xpath, namespaces=self.namespaces):
            ...
            # se agregan los siguientes condicionales para que expr modifique su valor y no imprima el display_name que
            # contenga 'Registro manual', como asi tambien, el guion siguiente, para que los names no inicien de la
            # siguiente manera: '- display_name'
            if expr == "__filter(move.display_name + (move.partner_id and ' - %s' % move.partner_id.name or ''))":
                expr = "__filter(move.display_name if 'Registro manual' not in move.display_name else '')"
            if expr == "__filter((move.ref and move.ref != move.name) and ' - %s' % move.ref or '')":
                expr = "__filter((move.ref and move.ref != move.name) and move.ref or '')"
            if is_opening:
